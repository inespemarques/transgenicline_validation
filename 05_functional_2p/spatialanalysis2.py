"""
=============================================================================
SPATIAL ANALYSIS - GABA DISTRIBUTION & LATERALITY (CORRECTED)
=============================================================================

Analyzes spatial patterns of GABA vs non-GABA neurons:
1. Left-Right distribution (laterality) - CORRECTED to use Y-axis
2. Rostro-caudal distribution (anterior-posterior axis) - X-axis
3. Regional clustering
4. Anatomical hotspots

CORRECT AXES:
- Y-axis: LEFT/RIGHT (midline at Y=215.8)
- X-axis: ROSTRAL/CAUDAL
...
class Config:
    ...
    OUT_DIR = r"C:\Users\OSVALDO\Downloads\2P_SPATIAL_ANALYSIS_CORRECTED"
    
    # Anatomical landmarks (CORRECTED)
    MIDLINE_Y = 215.8  # Y-axis midline for LEFT/RIGHT division
...
def load_classification_and_coords():
    ...
    # CORRECTED: Calculate laterality using Y-axis (distance from midline)
    df_class['distance_from_midline'] = df_class['centroid_y'] - cfg.MIDLINE_Y
    ...
    print(f"  Left hemisphere (Y < {cfg.MIDLINE_Y}): {(df_class['hemisphere'] == 'LEFT').sum():,}")
    print(f"  Right hemisphere (Y > {cfg.MIDLINE_Y}): {(df_class['hemisphere'] == 'RIGHT').sum():,}")
    
    return df_class


# =============================================================================
# 2. LATERALITY ANALYSIS (CORRECTED for Y-axis)
# =============================================================================

def analyze_laterality(df):
    """Analyze left-right distribution of GABA vs non-GABA."""
    print("\n" + "="*70)
    print("LATERALITY ANALYSIS (Y-AXIS)")
    print("="*70)
    ...
    for name, data in [('GABA', gaba), ('Non-GABA', non_gaba)]:
        ...
        print(f"\n  {name}:")
        print(f"    LEFT (Y < {cfg.MIDLINE_Y}):  {left_count:,} ({left_pct:.1f}%)")
        print(f"    RIGHT (Y > {cfg.MIDLINE_Y}): {right_count:,} ({right_pct:.1f}%)")
    ...
    # Distance from midline (absolute Y distance)
    print("\n[Distance from Midline (Y-axis)]")
...
def plot_laterality_analysis(df, laterality_results, save_dir):
    """Create laterality analysis figure (CORRECTED Y-axis)."""
    ...
    # === Panel A: Distribution across midline (Y-axis) ===
    ax1 = plt.subplot(2, 3, 1)
    
    bins = np.linspace(-cfg.MIDLINE_Y, cfg.IMAGE_HEIGHT - cfg.MIDLINE_Y, 60)
    ...
    ax1.set_xlabel('Distance from Midline (Y-axis, pixels)')
    ax1.set_ylabel('Density')
    ax1.set_title('A. Distribution Across Midline (Y-axis)', fontweight='bold')
    ax1.legend()
    ax1.grid(alpha=0.3)
    
    # Add L/R labels
    ax1.text(-100, ax1.get_ylim()[1]*0.9, 'LEFT', fontsize=12, 
             fontweight='bold', ha='center')
    ax1.text(100, ax1.get_ylim()[1]*0.9, 'RIGHT', fontsize=12,
             fontweight='bold', ha='center')
...
Distance from Midline (Y):
  ...
  p = {laterality_results['distance_from_midline']['p_value']:.2e}
    ...
    # CORRECTED: Midline is HORIZONTAL (Y-axis)
    ax4.axhline(cfg.MIDLINE_Y, color='white', linestyle='--', linewidth=2,
                label='Midline (Y=215.8)')
    ...
    ax4.set_xlabel('X Position (Rostral→, pixels)')
    ...
    # CORRECTED: Add L/R labels on Y-axis
    ax4.text(cfg.IMAGE_WIDTH - 50, cfg.MIDLINE_Y - 50, 'L', color='white', 
             fontsize=20, fontweight='bold', ha='center')
    ax4.text(cfg.IMAGE_WIDTH - 50, cfg.MIDLINE_Y + 50, 'R', color='white', 
             fontsize=20, fontweight='bold', ha='center')
    
    # Add anatomical labels
    ax4.text(50, 50, 'CAUDAL', color='white', fontsize=10, rotation=0)
    ax4.text(cfg.IMAGE_WIDTH - 50, cfg.IMAGE_HEIGHT - 50, 'ROSTRAL', 
             color='white', fontsize=10, rotation=0, ha='right')
    ...
    ax5.set_ylabel('Distance from Midline (Y-axis, pixels)')
    ...
    ax6.set_xlabel('Z Plane (Dorsal→Ventral)')
    ...
    plt.suptitle('Laterality Analysis (Y-AXIS): GABA vs Non-GABA Neurons',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    save_path = Path(save_dir) / "figures" / "laterality_analysis_CORRECTED.png"
...
# =============================================================================
# 3. ROSTRO-CAUDAL ANALYSIS (X-axis is correct)
# =============================================================================

def analyze_rostrocaudal(df):
    """Analyze rostro-caudal (X-axis) distribution."""
    print("\n" + "="*70)
    print("ROSTRO-CAUDAL ANALYSIS (X-AXIS)")
    print("="*70)
    ...
    # Statistical tests
    print("\n[X Position Distribution (Rostro-Caudal)]")
    ...
    print("\n[Z Plane Distribution (Dorso-Ventral)]")
...
{'Significant spatial differences detected!' if rc_results['x_distribution']['p_value'] < 0.05 
 or rc_results['z_distribution']['p_value'] < 0.05 
    ...
    save_path = Path(save_dir) / "figures" / "rostrocaudal_analysis_CORRECTED.png"
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def main():
    """Run spatial analysis pipeline with corrected axes."""
    
    print("="*80)
    print("SPATIAL ANALYSIS - CORRECTED AXES")
    print("="*80)
    print(f"\nCORRECT ORIENTATION:")
    print(f"  • Y-axis: LEFT/RIGHT (Midline at Y = {cfg.MIDLINE_Y})")
    print(f"  • X-axis: ROSTRAL/CAUDAL")
    print(f"  • Z-axis: DORSAL/VENTRAL")
    print(f"\nOutput: {cfg.OUT_DIR}")
    
    # Load data
    df = load_classification_and_coords()
    
    # Laterality analysis (Y-axis)
    laterality_results = analyze_laterality(df)
    plot_laterality_analysis(df, laterality_results, cfg.OUT_DIR)
    
    # Rostro-caudal analysis (X-axis)
    rc_results = analyze_rostrocaudal(df)
    plot_rostrocaudal_analysis(df, rc_results, cfg.OUT_DIR)  # ADD THIS PLOT!
    ...
    # Convert numpy types to native Python (IMPROVED)
    ...
    with open(Path(cfg.OUT_DIR) / "results" / "spatial_analysis_CORRECTED.json", 'w') as f:
        json.dump(all_results, f, indent=2)
    
    print("\n" + "="*80)
    print("✅ CORRECTED SPATIAL ANALYSIS COMPLETE")
    print("="*80)
    ...
    print(f"\n📁 Output: {cfg.OUT_DIR}")
    print(f"  • laterality_analysis_CORRECTED.png")
    print(f"  • rostrocaudal_analysis_CORRECTED.png")
    print(f"  • spatial_analysis_CORRECTED.json")
    print("="*80)
