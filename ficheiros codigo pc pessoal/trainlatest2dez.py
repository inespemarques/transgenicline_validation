# ===========================================================
# train_fish_swin3d_stable.py — SwinCell 3D no Windows (estável)
# ===========================================================
...
def main():
    ...
    print("[INFO] shape original =", img_shape)
    ...
    # imagens das augmentations já estão em [0,1]
    args.a_min = 0
    args.a_max = 1
    ...
    train_transform = transforms.Compose([
        ...
        transforms.ScaleIntensityRanged(
            keys=["image"], a_min=0, a_max=1, b_min=0, b_max=1, clip=True
        ),
    ...
    # =======================================================
    # 7. Dataset + DataLoader (ESTÁVEL: num_workers=0)
    # =======================================================
    ...
    train_loader = data.DataLoader(
        ...
        batch_size=1,
        shuffle=True,
        num_workers=0,   # 👈 sem multiprocessing no Windows → sem erro
        drop_last=True
    )
    ...
    for epoch in range(args.max_epochs):
        ...
        for batch in train_loader:
            ...
            lbl = batch["label"].to(device)   # (B,4,Z,Y,X)
        ...
        print(f"Epoch {epoch:03d} | loss={avg:.4f} | time={time.time()-t0:.2f}s")
        ...
        if (epoch + 1) % 10 == 0:
            save_checkpoint(
                ...
                best_acc=0, optimizer=optimizer
            )
            print(f"[CKPT] Modelo salvo na epoch {epoch}")
