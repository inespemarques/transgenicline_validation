...
# Cleaner spacing for publication: 1, 3, 7, 10 pixels
FOCUSED_SHELLS = [
    # (start, end) - ONLY pure exterior regions for clean comparison
    (0, 1),    # 1 pixel exterior
    (0, 3),    # 3 pixels exterior
    (0, 7),    # 7 pixels exterior
    (0, 10),   # 10 pixels exterior
]

# Fewer percentiles for speed
...
PERCENTILE_EXCLUDE_ZEROS = False
...
def create_shell_dilation(label_slice, dist_map, start_dist, end_dist):
    shell = np.zeros_like(label_slice, dtype=label_slice.dtype)
    
    # Nuclear region
    if start_dist < 0:
        nuclear_mask = (dist_map >= float(start_dist)) & (dist_map < 0)
        shell[nuclear_mask] = label_slice[nuclear_mask]
    ...
    if end_dist > 0:
        ...
        if start_dist <= 0:
            start_iter = 0
            end_iter = int(np.ceil(end_dist))
        else:
            start_iter = int(np.floor(start_dist))
            end_iter = int(np.ceil(end_dist))
        
        if end_iter > 0:
            outer_labels = label_slice.copy()
            for _ in range(end_iter):
                outer_labels = grey_dilation(outer_labels, footprint=struct)
        else:
            outer_labels = label_slice.copy()
        
        if start_iter > 0:
            inner_labels = label_slice.copy()
            for _ in range(start_iter):
                inner_labels = grey_dilation(inner_labels, footprint=struct)
        else:
            inner_labels = label_slice.copy()
        
        outer_mask = outer_labels > 0
        inner_mask = inner_labels > 0
        shell_region = outer_mask & (~inner_mask)
        
        shell[shell_region] = outer_labels[shell_region]
...
def process_slice(slice_key, method="voronoi"):
    ...
    if method == "voronoi":
        ...
        dist_map = precompute_dilation_signed_distance(mask)
        assigned = None
    ...
    for (start_dist, end_dist) in FOCUSED_SHELLS:
        ...
        for p, bright_thr in bright_thr_by_p.items():
            if method == "voronoi":
                ...
                shell = create_shell_dilation(mask, dist_map, start_dist, end_dist)
...
def plot_comparison_by_slice(all_results, out_path):
    """Bar plot comparing methods SEPARATELY for each slice"""
    
    # Filter for pure exterior shells
    selected_exterior = [1, 3, 7, 10]
    
    # Get data for each slice separately
    slices = sorted(all_results["slice"].unique())
    
    # LARGER figure for thesis
    fig, axes = plt.subplots(len(slices), 3, figsize=(26, 7.5 * len(slices)))
    ...
    # LARGER title
    fig.suptitle("Voronoi vs Dilation: Pure Exterior Shells BY SLICE\n(1, 3, 7, 10 pixel exterior - no nuclear component)",
...
def main():
    ...
    print(f"Testing {len(FOCUSED_SHELLS)} focused shell configurations")
